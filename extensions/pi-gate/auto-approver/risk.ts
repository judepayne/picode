...
import type { GateAutoApprovalRequest, GateAutoDecision } from "./types.ts";

export type GateAutoRiskFlag =
...
export interface GateAutoRiskAssessment {
	flags: GateAutoRiskFlag[];
	recommendedDecision: GateAutoDecision | "allow_if_clearly_requested";
	reason?: string;
...
function addFlag(flags: GateAutoRiskFlag[], flag: GateAutoRiskFlag): void {
	if (!flags.includes(flag)) flags.push(flag);
}

function commandText(request: GateAutoApprovalRequest): string {
	return `${request.bash?.command ?? ""}\n${request.subject}`;
}

function analysisText(request: GateAutoApprovalRequest): string {
	return stringifyLower(request.bash?.analysis ?? request.inputSummary ?? {});
}

function combinedText(request: GateAutoApprovalRequest): string {
