...
- **An OpenCode inspired permission system** for each agent/subagent, with optional `/gate auto on` semantic approval through `auto.json` hard-denies, optional always-allows, trusted sequential context, and role guidance. Auto approval can use a managed local llama.cpp backend or a model from Pi's configured model registry, and starts only when explicitly enabled for the session unless the project opts into `gate.auto.startOnSession=true`.
