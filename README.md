...
Optionally, `/gate auto on` enables semantic local auto-approval for the current Pi session. It uses `auto.json` hard-denies, optional role-specific always-allows, and per-agent/subagent natural-language guidance instead of policy `ask` rules. Policy denies and hard denies remain final, model allows are one-call-only, grey-area calls go to the semantic approver with trusted sequential context, and repeated model blocks pause into normal prompting. It does not auto-start after a Pi restart unless `gate.auto.startOnSession=true` is set. Users own the local `llama-server` and GGUF model artifacts; helpers (`scripts/setup-gate-auto-approver.mjs`, `scripts/eval-gate-auto-approver.mjs`, `scripts/smoke-gate-auto-soft-block.mjs`) only configure/verify them.
